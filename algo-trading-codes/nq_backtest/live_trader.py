# live_trader.py — Real-time NQ Algo Trading Engine for TopstepX
#
# This script connects to your TopstepX account via the ProjectX API,
# fetches 15-minute NQ candle data, evaluates your 5 strategies,
# and places bracket orders with TP/SL/trailing stop management.
#
# USAGE:
#   python live_trader.py              (live mode)
#   python live_trader.py --dry-run    (paper mode — signals only, no orders)
#
# REQUIREMENTS:
#   - .env file with PROJECT_X_API_KEY and PROJECT_X_USERNAME
#   - project-x-py SDK installed
#   - Your existing: config.py, indicators.py, strategies.py
...
DAILY_LOSS_LIMIT_POINTS = DAILY_LOSS_LIMIT_DOLLARS / \
    20  # = 50 NQ points (1 pt = $20)
SAFETY_BUFFER_POINTS = 10     # Stop trading when within 10 pts of limit
CONTRACTS = 3                  # Number of NQ contracts per trade
...
# Instrument
SYMBOL = "MNQ"   # NQ E-Mini NASDAQ 100 Futures
...
class LiveTrader:
    ...
    def print_daily_summary(self):
        ...
        self.log.info(
            f"  Total PnL: {self.daily_pnl:+.1f} pts (${self.daily_pnl * 2:+,.0f})")
